...
class Blogscon extends CI_Controller {
	...
	public function index()
	{
       ...
       $data=array();
		...
		$data['breadcrumbs'][] = array(
		'text' => 'blogs Manager',
		'href' => base_url().adminpath.'/blogscon'
		);
		$data['heading']="blogs Manager";
		$this->load->model(adminpath.'/model_blogscon');
		$data['allproducts']= array();	
    	$results = $this->model_blogscon->getproductss();
		if ($results) {  
			foreach($results as $val){
				...
				$data['allproducts'][] = array(
					...
					'image' => $image,
					'status' => $val->status,
					'href' => base_url().adminpath.'/blogscon/edit?id=' . $val->id
				);
			}
		}
		
		$data['deleteaction'] = base_url().adminpath.'/blogscon/delete';
		$data['activeaction'] = base_url().adminpath.'/blogscon/active';
		$data['inactiveaction'] = base_url().adminpath.'/blogscon/inactive';
		$this->load->view(adminpath.'/header');
		$this->load->view(adminpath.'/blogscon',$data);
    ...
    public function active(){
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			$this->load->model(adminpath.'/model_blogscon');
			$this->model_blogscon->active($_POST['table_records']);
    	}
    	redirect(adminpath.'/blogscon');
    }
    public function inactive(){

		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			$this->load->model(adminpath.'/model_blogscon');
			$this->model_blogscon->inactive($_POST['table_records']);
    	}
    	redirect(adminpath.'/blogscon');
    }
    public function delete(){
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			$this->load->model(adminpath.'/model_blogscon');
			$this->model_blogscon->delete($_POST['table_records']);
    	}
    	redirect(adminpath.'/blogscon');
    }
    public function add() {
		 ...
		 if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if($_FILES['bigimage']['name'] <>'')
		 	{
				...
				move_uploaded_file($_FILES['bigimage']['tmp_name'], DIR_IMAGE."blogs/".$expdoc_file) or $error = "Not Uploaded";
				$imgname="blogs/".$expdoc_file;
			...
			if($_FILES['image']['name'] <>'')
		 	{
				...
				$random_digit=rand(0000,9999);
				$expdoc_file=$random_digit.$expdoc_file;
				move_uploaded_file($_FILES['image']['tmp_name'], DIR_IMAGE."blogs/".$expdoc_file) or $error = "Not Uploaded";
				$imgname="blogs/".$expdoc_file;
			...
			$postdata['status'] = '1';
			$this->model_blogscon->add($postdata,$category);
			$this->session->set_flashdata('blogsnotify', 'Username and Password not Valid.');
			redirect(adminpath.'/blogscon');
		}
		$this->getform();
     ...
     public function edit() {
		 ...
		 if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			$postdata['status'] = '1';
			$this->model_blogscon->edit($this->input->get('id'),$postdata,$category);
			$this->session->set_flashdata('blogsnotify', 'Username and Password not Valid.');
			redirect(adminpath.'/blogscon');
		}
		$this->getform();
    ...
    public function getform() {
		...
		$data['heading']="Add blogs";
		$data['action'] = base_url().adminpath.'/blogscon/add';
		} else {
		$data['breadcrumbs'][] = array(
		'text' => 'Edit blogs',
		'href' => '#'
		);
		$data['heading']="Edit blogs";
		$data['action'] = base_url().adminpath.'/blogscon/edit?id=' . $this->input->get('id');
		}
		
		if ($this->input->get('id') && $this->input->server('REQUEST_METHOD') != 'POST') {
      		$info = $this->model_blogscon->getproduct($this->input->get('id'));
    	}
		...
		if ($results) {  
			foreach($results as $val){
					...
					foreach($subresults as $val1){
						...
						if ($subresults1) {
							foreach($subresults1 as $val2){
								$catchecked="";
								if($this->model_blogscon->getproductcategory($data['id'],$val2->id)){
									$catchecked="checked";
								} 
								$subcategory1[] = array(
									'id' => $val2->id,
						...
						$catchecked="";
							if($this->model_blogscon->getproductcategory($data['id'],$val1->id)){
								$catchecked="checked";
							} 
				...
				$catchecked="";
				if($this->model_blogscon->getproductcategory($data['id'],$val->id)){
					$catchecked="checked";
				}
				$data['allpcategory'][] = array(
					'id' => $val->id,
		...
		$data['cancel'] = base_url().adminpath.'/blogscon';
		
		$this->load->view(adminpath.'/header');
		$this->load->view(adminpath.'/blogsconform',$data);
		$this->load->view(adminpath.'/footer');
